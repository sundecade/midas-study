...
class APIRetriever:
    ...
    def _add_endpoint(self, data, section, subsection, path):
        """添加一个端点并构建搜索文本。"""
        command = data.get("request_command") or path
        ep = {
            "section": section,
        ...
        search_parts = [
            ...
            section,
            subsection,
            self._infer_cn_keywords(api_name, command, section, subsection, data.get("http_method", [])),
        ]
        # 参数描述
    ...
    def search(self, query, section=None, top_k=10):
        if not query.strip():
            ...
            for i, doc_text in enumerate(self.search_index):
                ...
                command_lower = ep.get("request_command", ep["path"]).lower()
                path_lower = ep["path"].lower()
                name_path_text = api_name_lower + " " + path_lower + " " + command_lower

                # ── 路径匹配（最高权重）──
                if query_original in (path_lower.lstrip("/"), command_lower.lstrip("/")):
                    score += 800
                elif query_original in path_lower or query_original in command_lower:
                    score += 300
                path_parts = (path_lower + " " + command_lower).replace("/", " ").replace("-", " ").split()
                for term in terms:
                    if len(term) >= 2:
    ...
    def get_by_path(self, path):
        """根据路径精确获取一个端点。"""
        for ep in self.endpoints:
            if ep["path"] == path or ep.get("request_command") == path:
                return ep
        return None
    ...
    def get_cn_display_name(self, ep):
        ...
        path = ep["path"]
        command = ep.get("request_command", path)
        ...
        if not cn_name:
            cn_name = self.PATH_TO_CN_NAME.get(path, "")
        if not cn_name:
            cn_name = self.PATH_TO_CN_NAME.get(command, "")
        # 兜底
        if not cn_name:
            cn_name = api_name[:40]
    ...
    def format_endpoint_detail(self, ep):
        ...
        d = ep["data"]
        lines = []
        command = ep.get("request_command", ep["path"])
        lines.append(f"## {d.get('api_name', '')} — `{command}`")
        if command != ep["path"]:
            lines.append(f"**条目**: {ep['path']}")
