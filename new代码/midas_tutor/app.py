    ...
    if st.session_state.get("_search_done"):
        if results:
            ...
            for ep, score in results:
                ...
                path = ep.get("request_command", ep["path"])
                section = ep['section']
    ...
    if browse_section:
        ...
        if subs:
                ...
                with st.expander(f"📁 {sub_name} ({len(sub_eps)} 个接口)"):
                    for ep in sub_eps[:30]:
                        ...
                        if isinstance(methods, str):
                            methods = [methods]
                        st.markdown(
                            f"**[{d.get('api_name', '')}][{cn_name}]** `{ep.get('request_command', ep['path'])}` — "
            ...
            for ep in eps:
                cn_name = retriever.get_cn_display_name(ep)
                en_name = ep['data'].get('api_name', '')
                with st.expander(f"**[{en_name}][{cn_name}]** — `{ep.get('request_command', ep['path'])}`"):
                    _render_endpoint_card(ep)
        ...
        if user_input or default_input:
            ...
            with st.chat_message("assistant"):
                try:
                    ...
                    if results:
                        refs = "\n\n---\n**📚 参考接口**: "
                        refs += " · ".join(f"`{ep.get('request_command', ep['path'])}`" for ep, _ in results[:5])
                        st.markdown(refs)
                        answer += refs
