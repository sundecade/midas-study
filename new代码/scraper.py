...
def _parse_endpoint_table(table):
    ...
    for i, header in enumerate(headers):
        compact = re.sub(r'[\s_-]+', '', header)
        if any(word in compact for word in ('endpoint', 'inputuri', 'uri', 'url', 'command')):
            endpoint_col = i
        if any(word in compact for word in ('detail', 'manual', 'description')):
            detail_col = i
    ...
    for row in rows:
        ...
        if detail_col is not None and detail_col < len(cells):
            detail_cell = cells[detail_col]
        else:
            detail_cell = next((cell for cell in cells if cell.find('a', href=True)), cells[-1])
        ...
        if endpoint_col is not None and endpoint_col < len(cells):
            endpoint_text = normalize_endpoint_path(cells[endpoint_col].get_text(" ", strip=True))

        if not endpoint_text:
            for cell in cells:
                candidate = normalize_endpoint_path(cell.get_text(" ", strip=True))
                if candidate:
                    endpoint_text = candidate
                    break

        # Some POST index tables do not have an endpoint column. Keep the row
        # unique here; parse_article_body() recovers the real command path from
        # the article's Input URI as request_command.
        if not endpoint_text:
            endpoint_text = name
...
def normalize_endpoint_path(raw_text):
    """Return a clean API command path such as /db/NODE or /post/TABLE."""
    if not raw_text:
        return ""
    ...
    # Article URI tables often say "{base url} + post/TABLE".
    plus_match = re.search(r'(?i)(?:\{?\s*base\s*url\s*\}?)\s*\+\s*([A-Za-z][A-Za-z0-9_/-]*)', text)
    if plus_match:
        text = plus_match.group(1)
        found_command = True

    # Keep only the command-looking portion if surrounding prose exists.
    path_match = re.search(r'(?i)(/?(?:doc|db|ope|view|post)/[A-Za-z0-9_./-]+)', text)
    if path_match:
        text = path_match.group(1)
        found_command = True

    text = text.strip().strip('`"\'')
    if not text or not found_command:
        return ""
    if not text.startswith('/'):
        text = '/' + text

    # MIDAS examples use a lower-case section prefix and upper-case command names.
    match = re.match(r'^/([^/]+)(/.*)$', text)
    if match:
        text = '/' + match.group(1).lower() + match.group(2).upper()
    return text
    ...
    if 'PUT' in http_methods:
        ...
        if not has_put:
            generated.append({
                ...
                "content": {"Assign": {"ID": 1}},
                "note": "PUT 请求在 Assign 中指定要修改对象的 ID 及新值。如: {\"Assign\": {\"1\": {\"ID\": 1, ...新参数值...}}}"
            })
...
def parse_article_body(article_data):
    ...
    if uri_section:
        ...
        if uri_table:
            ...
            result["request_command"] = normalize_endpoint_path(result["url"])
    ...
    if method_section:
        ...
        if method_table:
            ...
            methods = re.findall(r'(GET|POST|PUT|DELETE|PATCH)', method_text, flags=re.IGNORECASE)
            result["http_method"] = [m.upper() for m in methods] if methods else [method_text.strip().upper()]
