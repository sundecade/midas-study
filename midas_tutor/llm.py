...
CONSTRAINT 是 7 位字符串。GROUP_NAME 空字符串即可。**多个节点必须分开在不同的数字键下，不要全部放到一个 ITEMS 数组里。**
	🚨 关键：**Assign 下的第一级数字键才是节点号！** 示例中 Assign["1"]=节点1，Assign["11"]=节点11。
	ITEMS 内的 ID 字段只是约束记录的内部序号（1,2,3...），**和节点号无关，不要把它改成节点号！**
...
class LLMClient:
    ...
    def chat(self, messages, stream=False, temperature=0.3):
        ...
        kwargs = dict(model=self.model, messages=full_messages, temperature=temperature, max_tokens=4096)
        if self.extra_body:
...
12. 多个对象（多节点/多材料）必须放在独立的数字键下，不能堆在一个键里
	    Assign 下的数字键才是节点号/对象序号（如 Assign["11"]=给11号节点加约束）
	    ITEMS 内的 ID 只是内部序号，**不是节点号，不要改它！**
